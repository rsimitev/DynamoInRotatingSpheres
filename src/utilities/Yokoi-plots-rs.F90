...
#include "drsDefs.F90"
   ...
   call saveIDLmeridional(EMF, 0.0d0, 'EMF')
   ...
   call saveIDLmeridional(alphaBz, 0.0d0, 'alphaBz')
   ...
   call saveIDLmeridional(betaJz, 0.0d0, 'betaJz')
   ...
   call saveIDLmeridional(gammaOz, 0.0d0, 'gammaOz')
...
contains
   ...
   !> Save the requested meridional cut in a format compatible with
   !! Radostin's IDL scripts.
   subroutine saveIDLmeridional(field, phi, filename)
      ...
      double precision:: render_out(0:(blk_t_size(mpi_rank)-1),Nr, 2)
      double precision:: phi_n, dphi, dphi1, dphi2
      double precision:: phi_n2, dphi12, dphi22
      double precision:: mcut_min, mcut_max, mcut_rms
      double precision:: sliceVol
      integer:: i,l,icut, icut2

      ! dphi is the space between nodes
      dphi  = 2*pi/m0/Np

      ! The phi part
      ! *******************
      ! icut is the index of the node with azimuth smaller than phi
      icut  = int(phi/dphi) + 1
      ! phi_n is the azimuth of that node
      phi_n = (icut-1)*dphi
      ! dphi1 is the angular distance between phi and that node
      dphi1 = phi - phi_n
      ! dphi2 is the angular distance between phi and the next node
      dphi2 = dphi - dphi1

      ! The phi+pi part
      ! *******************
      ! icut2 is the index of the node antipodal to phi
      icut2  = Np/2 + icut
      ! phi_n2 is the azimuth of that node
      phi_n2 = (icut2-1)*dphi
      ! dphi1 is the angular distance between phi and that node
      dphi12 = phi+pi - phi_n2
      ! dphi2 is the angular distance between phi and the next node
      dphi22 = dphi - dphi12

      ! Interpolate in phi
      render_out(:,:,1) = (field(:,icut,:)*dphi2 + field(:,icut+1,:)*dphi1)/dphi
      render_out(:,:,2) = (field(:,icut2,:)*dphi22 + field(:,icut2+1,:)*dphi12)/dphi

      sliceVol = sum(dOmega(:,:))
      ! find minimum and maximum
      mcut_min = minval(render_out)
      mcut_max = maxval(render_out)
      mcut_rms = sum(render_out(:,:,1)**2*dOmega(:,:))
      ! The complocated expression for the area comes from the definition
      ! of dOmega = r**2 dr d\phi sin(\theta) d\theta, d\phi = 2*pi/Np
      mcut_rms = dsqrt(mcut_rms/sliceVol)

      ! Save min and max
      open(2,file=trim(filename)//'_mima.txt',STATUS="UNKNOWN")
      Write(2,*) mcut_min, mcut_max
      close(unit=2)
      open(2,file=trim(filename)//'_rms.txt',STATUS="UNKNOWN")
      Write(2,*) mcut_rms
      close(unit=2)

      ! An IDL dimensions file
      open(2,file='idl.dims',STATUS="UNKNOWN")
      write(2,*) Nr
      write(2,*) Nt
      write(2,*) Np
      write(2,*) m0
      ...
      ! And now the content
      open (2,file=trim(filename)//'.x',STATUS="UNKNOWN")
      ! write the eastern part
      do l=0,Nt
         do i=1,Nr
            write(2,*) rcoll(i)*sqrt(1-costheta(l)**2)
         enddo
      enddo
      ! write the western part
      do l=Nt,0,-1
         do i=1,Nr
            write (2,*) -rcoll(i)*sqrt(1-costheta(l)**2)
         enddo
      enddo
      ! repeat first pencil to wrap the 2D array (the z-values)
      l=0
      do i=1,Nr
         write (2,*) rcoll(i)*sqrt(1-costheta(l)**2)
      enddo
      ...
      open (2,file=trim(filename)//'.y',STATUS="UNKNOWN")
      ! write the eastern part
      do l=0,Nt
         do i=1,Nr
            write (2,*) rcoll(i)*costheta(l)
         enddo
      enddo
      ! write the western part
      do l=Nt,0,-1
      ...
      open (2,file=trim(filename)//'.z',STATUS="UNKNOWN")
      ! write the eastern part
      do l=0,Nt
         do i=1,Nr
            write (2,*) render_out(l,i,1)
         enddo
      enddo
      ! write the western part
      do l=Nt,0,-1
         do i=1,Nr
            write (2,*) render_out(l,i,2)
         enddo
      enddo
   ...
   end subroutine saveIDLmeridional
